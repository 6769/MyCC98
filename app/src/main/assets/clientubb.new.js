...
/*
	...
	2016.12.06(樱桃): HTML5 SandBox 支持
*/
...
function searchubb(tagid, posttype, thetdclass) {
	...
	tdclass = thetdclass;
	var UbbInnerHtml = $(tagid).text();
	UbbInnerHtml = htmlEncode(UbbInnerHtml);
	if (posttype == 3)
...
function ubbcode(str) {	
	...
	var pattern;
	//noubb
	pattern = /(^[\s\S]*?)\[noubb\]([\s\S]*?)\[\/noubb\]([\s\S]*$)/i;
	...
	// code
	pattern = /(^[\s\S]*?)(\[code([\s\S]*?)\]([\s\S]*?)\[\/code\])([\s\S]*$)/i;
	while (pattern.test(str)) {
...
// SandBox 替换标签
function sandBoxReplaceFunc(all, params, content){
	...
	if (supportSandBox()){
		...
		var realParams = parseAndFilterArgs(params, paramTable, defaultValueTable);

		var ele = document.createElement('iframe');
		ele.sandbox = "allow-scripts allow-forms allow-same-origin";
		ele.src = realParams['url'];
		ele.style.border = 'none';
		ele.style.width = realParams['width'];
		ele.style.height = realParams['height'];
...
function ubbSandBox(str) {
	return parseLabel(str, 'sandbox', sandBoxReplaceFunc);
}
...
function ubbsigncode(str) {
	var pattern = /(^[\s\S]*?)\[noubb\]([\s\S]*?)\[\/noubb\]([\s\S]*$)/i;
	while (pattern.test(str)) {
		var beforeNoubb = RegExp.$1;
		ubb.storage.noubb.push(RegExp.$2);
		var afterNoubb = RegExp.$3;
...
var codeUbb = function (source, language) {
	...
	// 将空格替换为 tab
	source = source.replace(/^( +)([\s\S]*)$/mgi, function (all, space, rest) {
...
var ubb = {
	...
	pm: function (str) {
		str = str.replace(/\[pm=(.[^\[\'\"\:\(\)\;]*?)\]([\s\S]*?)\[\/pm\]/gi, "<a href=\"javascript:;\" onclick=\"window.open(\'messanger.asp?action=new&touser=$1\',\'new_win\',\'width=500,height=400,resizable=yes,scrollbars=1\')\">$2</a>");
	...
	table: function (str) {
		...
		return str.replace(/\[table(?:\=(\d+))?]([\s\S]*?)\[\/table]/gi, replaceTable);
	},
	...
	tr: function (str) {
		...
		return str.replace(/\[tr]([\s\S]*?)\[\/tr]/gi, replaceTR);
	},
	...
	th: function (str) {
		...
		return str.replace(/\[th(?:\=(\d+),(\d+))?]([\s\S]*?)\[\/th]/gi, replaceTH);
	},
	...
	td: function (str) {
		...
		return str.replace(/\[td(?:\=(\d+),(\d+))?]([\s\S]*?)\[\/td]/gi, replaceTD);
	},
	...
	url: function (str, signatureMode) {
		...
		str = str.replace(/\[url=(.[^\[\'\"\(\)]*)\]([\s\S]*?)\[\/url\]/gi, function (all, g1, g2) { return generateFunc(g1, g2, false, !signatureMode, null); });
		//自动识别网址
	...
	share: function (str) {
		str = str.replace(/\[share=([A-Za-z0-9]*)\]([\s\S]*?)\[\/share\]/gi, '<img src="' + icondir + 'file.gif" border="0" /><a href="http://share.cc98.org/$1" target="_blank">点此下载 $2</a>');
	...
	topic: function (str) {
		str = str.replace(/\[topic=(\d+),(\d+)\]([\s\S]*?)\[\/topic\]/gi, "<a href=\"/dispbbs.asp?boardid=$1&id=$2\">$3</a>");
		return str;
	},
	board: function (str) {
		str = str.replace(/\[board=(\d+)]([\s\S]*?)\[\/board\]/gi, "<a href=\"/list.asp?boardid=$1\">$2</a>");
		return str;
	},
	filter: function (str) {
		str = str.replace(/\[SHADOW=*([0-9]*),*(#*[a-z0-9]*),*([0-9]*)\]([\s\S]*?)\[\/SHADOW]/gi, "<table width=$1 ><tr><td style=\"filter:shadow(color=$2, strength=$3)\">$4</td></tr></table>");
		str = str.replace(/\[GLOW=*([0-9]*),*(#*[a-z0-9]*),*([0-9]*)\]([\s\S]*?)\[\/GLOW]/gi, "<table width=$1 ><tr><td style=\"filter:glow(color=$2, strength=$3)\">$4</td></tr></table>");
		return str;
	},
	subscription: function (str) {
		return str.replace(/\[subscription=(.[^\[\'\"\:\(\)\;]*?)\]([\s\S]*?)\[\/subscription\]/gi, "<a href=\"javascript:;\" onclick=\"window.open(\'subscription.asp?action=adduser&topicid=$1\',\'new_win\',\'width=500,height=400,resizable=yes,scrollbars=1\')\">$2</a>");
	},
	nothot: function (str) {
		return str.replace(/\[nothot\](.[^\[]*)\[\/nothot\]/gi, "<span name=\"nothot\">$1</span>");
	},
	font: function (str) {
		return str.replace(/\[font=(.[^\[\'\"\:\(\)\;]*?)\]([\s\S]*?)\[\/font\]/gi, '<span style="font-family:$1;">$2</span>');
	},
	list: function (str) {
		str = str.replace(/\[\*\]/gi, "<li>");
		str = str.replace(/\[list\]([\s\S]*?)\[\/list\]/gi, "<ul>$1</ul>");
		str = str.replace(/\[list=(1|a|A)\]([\s\S]*?)\[\/list\]/gi, "<ul type=\"$1\">$2</ul>");
		return str;
	},
	align: function (str) {
		str = str.replace(/\[float=(left|right)\]([\s\S]*?)\[\/float\]/gi, "<br style=\"clear: both\"><span style=\"float:$1;\">$2</span>");
		str = str.replace(/\[align=(center|left|right)\]([\s\S]*?)\[\/align\]/gi, "<div align=\"$1\">$2</div>");
		str = str.replace(/\[right\]([\s\S]*?)(\[\/right\])/gi, "<div align=\"right\">$1</div>");
		str = str.replace(/\[left\]([\s\S]*?)(\[\/left\])/gi, "<div align=\"left\">$1</div>");
		str = str.replace(/\[center\]([\s\S]*?)(\[\/center\])/gi, "<div align=\"center\">$1</div>");
		return str;
	},
	size: function (str) {
		var pattern;
		pattern = /\[size=([0-9]*)(pt|px)*\]([\s\S]*?)\[\/size\]/i;
		while (pattern.test(str)) {
			var unit = RegExp.$2;
			var size = RegExp.$1;
			if (unit == '') {
	...
	md: function (str) {
		return str.replace(/\[md]([\s\S]*)\[\/md]/ig, function (all, content){
			dynamicMdCount++;
			var id = String.format("md-dynamic-{0}", dynamicMdCount);
			return String.format('<div id="{0}" class="md-read md-read-dynamic" style="background-color: transparent;"><textarea style="display: none;">{1}</textarea></div>', id, content);
		});
	},
	sandbox: function(str){
		return ubbSandBox(str);
	}
}